...
2. Check the working tree and preserve unrelated edits, uploads, and migrations.
   Frontend migration work targets `frontend-next/`; check
   [migration status](frontend.md#migration-status) and its `AGENTS.md` first.
...
| Change | Verification |
| --- | --- |
| Next.js frontend | `npm run lint` and `npm run build` in `frontend-next/`; affected interaction when a browser is available |
| Legacy frontend | `npm run build` in `frontend/`; affected interaction when a browser is available |
