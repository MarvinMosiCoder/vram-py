...
import ApplicationLogo from "@/lib/ApplicationLogo";
...
const AppNavbar = () => {
  ...
  const [unreadnNotifications, setUnreadnNotifications] = useState(0);
  ...
  useEffect(() => {
    api
      .get<{ notifications?: AdminNotification[]; unread_count?: number }>("/notification/notifications")
      .then((response) => {
  ...
  const readNotification = async (id: number) => {
    ...
    try {
      ...
      console.error("Failed to mark as read:", axios.isAxiosError(error) ? error.response?.data || error.message : error);
    }
  };
    ...
    <>
      <div className="min-h-20 pt-10 text-skin-text md:min-h-0 md:pt-0 relative flex shrink-0 items-center justify-between border-b border-skin-border bg-skin-panel px-4 md:px-7">
        ...
        <div className="flex min-w-0 items-center gap-3.5">
          ...
          <button type="button" aria-label="Open notifications" aria-expanded={showNotifications} className="relative cursor-pointer border-0 bg-transparent p-0 text-inherit" onClick={handleToggleNotification}>
          ...
          <div className="relative" ref={menuRef}>
            ...
            {showMenu && (
              <div
                className="absolute right-0 top-[calc(100%+10px)] z-150 flex w-75 flex-col gap-0 rounded-[10px] border border-skin-border bg-skin-panel py-3.5 shadow-[0_12px_32px_rgba(0,0,0,0.35)]"
                role="menu"
              >
