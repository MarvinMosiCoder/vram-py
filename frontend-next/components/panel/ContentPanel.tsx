...
const ContentPanel = ({ title, children, footer, onClose, as: Tag = "div", className = "", ...props }: { title?: string; children: ReactNode; footer?: ReactNode; onClose?: () => void; as?: "div" | "form"; className?: string; onSubmit?: FormEventHandler<HTMLFormElement> }) => {
    return (
        <Tag className={`flex flex-col gap-4 rounded-[10px] border border-skin-border bg-skin-panel p-5 ${className}`.trim()} onSubmit={props.onSubmit as FormEventHandler<HTMLElement>}>
            {(title || onClose) && (
