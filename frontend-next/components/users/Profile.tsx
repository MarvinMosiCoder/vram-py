...
import { useEffect, useState, type ChangeEvent, type FormEvent, type KeyboardEvent, type MouseEvent } from "react";
...
// GET /profiles returns this user's upload history, newest first.
type SavedProfile = { id: number; file_name: string };
type ProfileAction = "download" | "delete" | "update";
type ProfileResponse = { message?: string; status?: string; file_name?: string };
...
async function profileErrorMessage(error: unknown, fallback: string) {
...
export default function Profile() {
    ...
    const { textColorActive, scrollbarTheme, primayActiveColor, borderTheme } = useThemeStyles(theme);
    const [profileImage, setProfileImage] = useState<string | null>(null);
    const { handleToast } = useToast();
    const [forms, setForms] = useState<{ profile_image: File | string }>({
    ...
    const [chooserTab, setChooserTab] = useState<"avatars" | "upload">("avatars");
    const [selectedAvatar, setSelectedAvatar] = useState<string | null>(null);
    const [profiles, setProfiles] = useState<SavedProfile[]>([]);
    const [profileUpdate, setProfileUpdate] = useState<number | null>(null);
    const isDark = theme === "bg-skin-black";

    const fetchProfiles = () => {
        api.get<SavedProfile[]>("/profiles")
    ...
    useEffect(() => {
        fetchProfiles();
        // Load once on mount, as in the React page; later refreshes follow saves.
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, []);

    // RequireAuth renders nothing until /me resolves, so this only narrows the type.
    if (!user) return null;

    const initials = getInitials(user.name ?? "");
    const backgroundColor = colorMap[initials.charAt(0) as keyof typeof colorMap] || theme;
    ...
    const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
        ...
        setLoading(true);
        try {
            const response = await api.post<ProfileResponse>("/save-edit-image", forms, {
                headers: {
                    "Content-Type": "multipart/form-data",
                },
            });

            handleToast(response.data.message || "Profile updated.", (response.data.status as "success" | "error") || "success");
    ...
    const handleAvatarSubmit = async () => {
        ...
        setLoading(true);
        try {
            const response = await api.post<ProfileResponse>("/save-profile-avatar", { avatar: selectedAvatar });
            handleToast(response.data.message || "Profile updated.", (response.data.status as "success" | "error") || "success");
            if (response.data.status === "success") {
                setProfile(response.data.file_name);
                setSelectedAvatar(null);
    ...
    const handleUpdateProfile = (e: MouseEvent | KeyboardEvent, id: number) => {
        e.preventDefault();
        setProfileUpdate(id);
    };

    const handleProfileUpdate = async (e: MouseEvent, id: number | null, action: ProfileAction) => {
        e.preventDefault();
        e.stopPropagation();
        if (loading) return;
        setLoading(true);
        ...
        try {
            const config = {
                ...
                responseType: action === "download" ? "blob" as const : "json" as const,
            };
            ...
            if (action === "download") {
                ...
                handleToast(data.message || "Profile updated.", (data.status as "success" | "error") || "success");
                if (data.status === "success") {
                    if (data.file_name) {
                        setProfile(data.file_name);
                    }
                    // null clears the avatar; ThemeContext reserves undefined for "follow /me".
    ...
    // Images are data-URL previews or runtime uploads with no fixed size,
    // which next/image cannot optimize, so plain <img> is kept throughout.
                            ...
                            <div className="p-5 md:p-6">
                                ...
                                <div className="grid gap-2 md:grid-cols-3">
                                    <div className="flex min-w-0 items-center gap-3 rounded-lg border border-(--das-border) bg-(--das-surface-muted) p-3">
                                        ...
                                        <span className="min-w-0"><span className="block text-[9px] font-semibold uppercase tracking-wide text-(--das-text-soft)">User ID</span><span className="mt-0.5 block text-xs font-semibold text-(--das-text)">#{user.id}</span></span>
                                    </div>
                                    <div className="flex min-w-0 items-center gap-3 rounded-lg border border-(--das-border) bg-(--das-surface-muted) p-3">
            ...
            >
                <div className="font-poppins">
                    <div className={`mb-5 grid grid-cols-2 rounded-lg p-1 ${isDark ? "bg-gray-900" : "bg-gray-100"}`}>
                        {([{ id: "avatars", label: "Choose avatar", icon: Images }, { id: "upload", label: "Upload image", icon: Upload }] as const).map((tab) => {
                            ...
                            profiles.map((item) => {
                                ...
                                return (
                                    <div
                                        ...
                                        key={item.id}
                                        onClick={(e) => handleUpdateProfile(e, item.id)}
                                        onKeyDown={(e) => {
                                            if (e.key === "Enter" || e.key === " ") {
                                                handleUpdateProfile(e, item.id);
                                            }
                                        }}
                                    >
