...
export default function SidebarMenuCardMultiple({ menuTitle, icon, childMenus }: { menuTitle: string; icon?: string | null; childMenus: SidebarMenu[] }) {
  const pathname = usePathname();
  const isActivePath = (href: string) => pathname === href || pathname.startsWith(`${href}/`);
  const hasActiveChild = (childMenus || []).some((child) => isActivePath(`/${child.path || child.slug}`));
  const [expansion, setExpansion] = useState({ pathname, open: hasActiveChild });
  const isOpen = expansion.pathname === pathname ? expansion.open : hasActiveChild;

  return (
    <div>
      <button
        type="button"
        onClick={() => setExpansion({ pathname, open: !isOpen })}
        aria-expanded={isOpen}
        className={`group mt-0! bg-transparent! flex w-full items-center overflow-hidden rounded-lg border px-2.5 py-2 text-[12px] font-semibold transition select-none ${
          hasActiveChild
      ...
      >
        {(childMenus || []).map((child) => {
          const href = `/${child.path || child.slug}`;
